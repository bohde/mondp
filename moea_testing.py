...
class TestIndividualDomination(unittest.TestCase):
    ...
    def setUp(self):
        self.one = eps_moea.moea_indiv()
        self.two = eps_moea.moea_indiv()
...
def testIndWrapper(n):
    class testIndiv(eps_moea.moea_indiv):
        fg = fitness_generator(n)

        def __init__(self):
            eps_moea.moea_indiv.__init__(self)
            self.mins = [0] * n
            self.eps = [.1] * n
...
class TestPopulation(unittest.TestCase):
    def setUp(self):
        ...
        self.pop = eps_moea.moea_pop(3, self.genome)
        self.pop.initialize()
...
class TestPopulationArchiveAcceptance(TestPopulation):
    ...
    def test_euclid_fail(self):
        ...
        o.fitness = (.98, .1)
        o.gen_ident()
        self.assertEquals(eps_moea.euclid_dist(o.ident_array, o.fitness) > eps_moea.euclid_dist(p.ident_array, p.fitness), True)
    ...
    def test_euclid_pass(self):
        ...
        o.fitness = (.98, .1)
        o.gen_ident()
        self.assertEquals(eps_moea.euclid_dist(o.ident_array, o.fitness) > eps_moea.euclid_dist(p.ident_array, p.fitness), False)
...
class TestEuclid(unittest.TestCase):
    def test(self):
        self.assertEquals(eps_moea.euclid_dist((0, 1), (1, 0)), 2)

    def test2(self):
        self.assertEquals(eps_moea.euclid_dist((2, 2), (0, 0)), 4)
